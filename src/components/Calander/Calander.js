...
function Calendar({ selectedProcedure, doctorId }) {
  ...
  async function onPay() {
    ...
    const data = await fetch("http://65.0.30.13:8080/consultationpayment", {
      method: "POST",
      timeout: 0,
      headers: {
  ...
  const fetchBookedAppointments = async () => {
    try {
      ...
      const response = await axios.get(
        `http://65.0.30.13:8080/consultation/booked/${docId}`
      );
  ...
  const saveToDatabase = async (date, time) => {
    ...
    try {
      const response = await axios.post(
        "http://65.0.30.13:8080/api/patients/consultation",
        {
