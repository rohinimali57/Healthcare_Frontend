...
function AddInsuranceDetailsArea() {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await fetch(`http://65.0.30.13:8080/accommodation/${pId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://65.0.30.13:8080/provider-insurance', {
    ...
    const handleDelete = async (index) => {
        ...
        try {
            const response = await axios.delete(`http://65.0.30.13:8080/provider-insurance/${insuranceId}`);
            console.log(response.data);
            Swal.fire({
                title: 'Success!',
