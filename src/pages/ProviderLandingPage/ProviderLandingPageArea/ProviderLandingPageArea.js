...
function ProviderLandingPageArea() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        await fetch(
          `http://65.0.30.13:8080/provider/procedure/search/${storedPId}`
        )
    ...
    const fetchprocedure = async () => {
      try {
        ...
        if (selectedProcedurelist) {
          await fetch(
            `http://65.0.30.13:8080/provider/procedures/search/${selectedProcedurelist}`
          )
            .then((res) => res.json())
  ...
  const handleDelete = async (id) => {
    ...
    const response = await axios.delete(`http://65.0.30.13:8080/procedures/delete/${id}`);
