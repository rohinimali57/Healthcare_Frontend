...
// get the procedure name 
 ...
 const fetchProcedureName = async () => {
      try {
        const response = await axios.get(`http://65.0.30.13:8080/procedure/name/${procedureId}`);
...
//       try {
//         const response = await axios.get(`http://65.0.30.13:8080/Procedure/providers/${procedureId}`);
...
useEffect(() => {
  const fetchProceduresData = async () => {
    try {
      const response = await fetch("http://65.0.30.13:8080/api/doctors/procedures", {
        method: "POST",
        headers: {
