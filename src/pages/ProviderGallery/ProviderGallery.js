...
const ProviderGallery = () => {
    ...
    const fetchData = async (providerId) => {
        try {
            const procedureResponse = await axios.get(`http://65.0.30.13:8080/procedure-images/${providerId}`);
            ...
            const doctorResponse = await axios.get(`http://65.0.30.13:8080/doctor-images/${providerId}`);
            console.log("doctor response", doctorResponse.data); // Check the response object
            setDoctorImages(doctorResponse.data.images);

            const providerResponse = await axios.get(`http://65.0.30.13:8080/provider-image/${providerId}`);
