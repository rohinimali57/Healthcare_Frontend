...
function ConsultationComponent() {
  ...
  useEffect(() => {
    const fetchConsultations = async () => {
      try {
        const response = await fetch(`http://65.0.30.13:8080/consultation/${patientId}`);
...
//payment 
  ...
  const handleDelete = async (consultationId) => {
    try {
      ...
      const response = await fetch(`http://65.0.30.13:8080/consultation/delete/${consultationId}`, {
        method: "DELETE",
      });
  ...
  async function onPay() {
    ...
    const data = await fetch('http://65.0.30.13:8080/payment', { 
      method: 'POST' ,
      timeout:0,
      headers:{
