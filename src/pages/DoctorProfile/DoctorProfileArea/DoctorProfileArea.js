...
const DoctorProfileArea = () => {
  ...
  //       fetch(`http://65.0.30.13:8080/doctor/delete/${index}`, {
  //         method: "DELETE",
  //         headers: {
  ...
  const handleClickOpen = async (id) => {
    ...
    fetch(`http://65.0.30.13:8080/doctor/delete/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        await fetch(
          `http://65.0.30.13:8080/provider/doctors/search/${storedPId}`
        )
    ...
    const fetchprocedure = async () => {
      try {
        ...
        if (selectedoctorlist) {
          await fetch(
            `http://65.0.30.13:8080/provider/doctor/search/${selectedoctorlist}`
          )
            .then((res) => res.json())
